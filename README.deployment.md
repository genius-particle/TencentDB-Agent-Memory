...
第三种为 **可选开源服务骨架**（Postgres+pgvector 记忆 + Postgres metadata；standalone 对象存储默认本地盘；Redis 状态后端），通过 `STORE_MODE=postgres` 显式选择记忆后端，**不改变** `deployMode=service` 的线上默认。见 [docs/store-open-backends.md](docs/store-open-backends.md) 与 [deploy/compose/open-service.yml](deploy/compose/open-service.yml)。K8s 安装用 [deploy/helm/tdai-memory](deploy/helm/tdai-memory)（`values.yaml` 即 Postgres sample；MinIO 需 `--set minio.deploy=true`）。
