...
第三种为 **可选开源服务骨架**（Postgres+pgvector 记忆 + Postgres metadata；standalone 对象存储默认本地盘，不设 `STORAGE_BACKEND`；本 fork 状态后端为 local），通过 `STORE_MODE=postgres` 显式选择记忆后端，**不改变** `deployMode=service` 的线上默认（仍是 tcvdb+cos+redis+mongo）。见 [docs/store-open-backends.md](docs/store-open-backends.md) 与 [deploy/compose/open-service.yml](deploy/compose/open-service.yml)。K8s 安装用 [deploy/helm/tdai-memory](deploy/helm/tdai-memory)（GHCR；SQLite 试用叠 `-f values-sqlite.yaml`；MinIO 需 `--set minio.deploy=true`）。
